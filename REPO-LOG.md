...
- deployed: **not deployed.** Committed `a48689f` and pushed to `claude/cta-examples-research-084f91`; NOT merged by design. A live "Become a client" button that collects a prospect's details and then goes quiet is worse than no button, and the EDDM drop has not mailed, so nothing is lost by holding for the e-signature decision.

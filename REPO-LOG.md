...
- deployed: **not deployed.** Both commits sit on `claude/marketingsite-iphone-preview-b66188`, unpushed, at David's explicit direction ("say the word when you want it published"). Nothing is on the apex. **OWNER: David** — this is a hold, not a blocker.
- screenshot: **Could not capture** — the change is human-visible and therefore owes a production capture, but it is not deployed. The captures shown to David in-session are local, and a local capture is not production proof. A real apex capture is owed the moment this ships.

...
### 2026-08-07 — WHAT/WHY/WHO/HOW layout directions (design not yet chosen; nothing shipped)
**Agent:** Claude (Claude Code, Opus 5) | **Surface:** marketing (cross-site: YAPTM + SWW) | **Duration:** ~1 session
- context: David asked for a WHAT/WHY/WHO/HOW section on both youarepayingtoomuch.com and smarterwaywealth.com, a full-bleed three-link CTA divider consistent across every page of both sites, a `Fee Calculator` nav item on both, and a stylized Smarter Way Wealth link at the far right of the YAPTM nav. **None of that is built.** This session produced design candidates only.
- rejected (first attempt): I built a mockup by lifting the CSS wholesale from Codex's unmerged `codex/wwwh-yaptm` prototype — Georgia serif spine, two-column grid with a hairline rule, arbitrary `min-height` leaving copy stranded in the corner. David rejected it outright ("terrible design… you obviously just copied off of Codex"). He was right; the artifact published at that stage is superseded and should not be used.
- added: `src/app/wwwh-preview/` — a temporary, noindex, sitemap-absent review route rendering the four answers in three candidate layouts using the real site font and palette (the earlier standalone mockup used a system-font stand-in, which is part of why the type read wrong). **Delete this route before merge.**
- changed: WHAT answer copy now reads "An investment and financial planning relationship with an experienced, highly credentialed advisor — for just $100 a month." David proposed dropping the "Smarter Way Wealth provides" opener; agreed, because the vertical question word is the grammatical subject, so restating the firm made WHAT the only answer of the four that didn't answer its own question. This also removed the only "SmarterWay" (no space) misspelling; a sweep confirms that variant appears nowhere else in either repo.
- bug found and fixed in my own work: the answer measure was set as `max-width: <n>ch` on a 16px container while the type renders at ~48px, collapsing every answer to a ~190px column (one or two words per line). Measure now lives on the element that carries the type size.
- candidates: **A (anchored)** — word and copy vertically centred against each other so balance holds regardless of answer length; airy, ~1.5 screens. **C (ledger)** — hairline opens each band, word hangs from it, tighter rhythm, ~half the height. **B (oversized watermark word)** built and cut: at the size that made it a graphic it bled across bands and competed with the sentence. B remains in the preview route as rejected evidence, not as a candidate.
- unresolved: David's reference screenshot shows a vertical serif "WHY" beside copy that matches **live** smarterwaywealth.com/save, but the live page has no vertical-text device anywhere in its markup and neither does that repo's source. The device exists only in Codex's local prototypes. Provenance never resolved — treat "same font size as /save" as referring to the vertical word, which is how it was implemented, but confirm with David.
- collision risk (needs David's decision): Codex has parallel unfinished WWWH work on **both** repos — `codex/wwwh-yaptm` (2 commits) and `codex/wwwh-sww` (3 commits). Neither is pushed, neither has a PR, and both cover roughly 40% of the brief (no FAQ link, no nav changes, divider on the homepage only). Building this branch out without retiring those guarantees two competing versions of the same section.
- evidence: local design captures at 1440px shown to David for a decision. `npm run build`, lint, and the existing test suites were **not run** — the only code added is a throwaway preview route with no production call sites, and no shipping component was touched.
- run-log: n/a — no batch/agent run; single interactive session.
- deployed: not deployed. Two commits on `claude/website-wwwh-sections-f90b0b`, not pushed, no PR. Neither production site has changed.
- next (blocked on David): pick A or C — everything downstream (both homepages, both navs, the site-wide CTA bar) inherits that choice. Then retire Codex's two branches, build both sites, delete the preview route. Also outstanding: the second screenshot David referenced for the YAPTM insertion point never arrived; placement below the calculator and above the Meet David block is an assumption.

### 2026-08-06 — Site-wide CTA divider above the footer (YAPTM side)
**Agent:** Claude (Claude Code) | **Surface:** YAPTM site-wide layout + test suite
- changed: the shipped `WwwhCtaDivider` (previously homepage-only) now renders immediately above `SiteFooter` on every public route via a new `SiteCtaDivider` client component mounted in `src/app/layout.tsx`. `WwwhCtaDivider` gained an optional `location` prop (default `"home_wwwh_divider"`, unchanged for the homepage call site); the site-wide mount passes `"site_footer_divider"` so PostHog can tell homepage clicks from site-wide clicks apart. `SiteCtaDivider` suppresses itself on `/` (the homepage already renders its own instance) and on the same route prefixes `SiteFooter` excludes, both now sourced from one shared `src/config/suppressedRoutePrefixes.ts` so the two components can't drift apart — a real drift (the divider briefly rendering orphaned on `/gallery` with no footer beneath it) was caught and fixed earlier in this same workstream.
- added: `tests/sitewide-cta-divider.mjs` (`npm run test:sitewide-divider`) — builds production, runs `next start`, walks every `src/app` route, and asserts exactly one correctly-tagged divider on every non-suppressed route, zero dividers on suppressed routes, and now also that the divider precedes `<footer>` in the raw server-rendered HTML on standard routes, proving `SiteCtaDivider` mounts immediately above the footer rather than just somewhere on the page. The route walker matches `page.(tsx|ts|jsx|js|mdx)`, not only `page.tsx`, so a future non-`.tsx` route can't silently drop out of coverage while the walker still looks complete.
- fixed (review round): `tests/home-wwwh.mjs`'s analytics-contract assertion had gone stale against this branch's own change (the `home_wwwh_divider` literal moved from a hardcoded JSX attribute into the `location` prop's default) and was RED on this branch. Replaced the dead regex with two assertions that separately prove the default value and the `{location}` wiring; both mutation-tested (temporarily broke each one individually, confirmed red, restored byte-identical, confirmed green). Added `npm run test:home-wwwh` so this guard stops being invisible. The new footer-order assertion is deliberately scoped to standard routes only: `src/app/loading.tsx` wraps every route's page content in a root Suspense boundary, and on `/` the homepage's own divider (embedded inside `CostAnalysisCalculator`, inside that boundary) streams into the raw HTML after `<footer>` even though it resolves well above the footer in the final DOM — a raw string-index comparison there would be a false failure unrelated to the actual `SiteCtaDivider`/`SiteFooter` mount order this check guards.
- verified locally: `node --test tests/home-wwwh.mjs` 10/10 (both new assertions mutation-tested); `npm run test:sitewide-divider` all routes green, including a temporary `page.jsx` route created solely to prove the broadened route-file pattern actually picks up non-`.tsx` pages (removed before commit); mount-order mutation check (temporarily swapped `<SiteCtaDivider />`/`<SiteFooter />` in `layout.tsx`) confirmed the new footer-order assertion fails on the first standard route checked, then reverted byte-identical and reconfirmed green; `npm run build` and `npm run lint` clean (0 errors, only pre-existing `<img>` warnings).
- not deployed: committed to `claude/website-wwwh-sections-f90b0b` only, per explicit instruction not to push, merge, or open a PR. This is Task 1 of the two-repo plan in `docs/superpowers/plans/2026-08-06-sitewide-cta-divider.md`; the smarterwaywealth.com side (Tasks 3/4, separate worktree) is not covered by this entry.
- deferred (unchanged): `SiteNav.tsx` and `ProgressiveStickyBar.tsx` each still carry their own duplicate 4-prefix suppressed-route list instead of reading `suppressedRoutePrefixes.ts`; noted as a consolidation candidate but intentionally out of scope for this pass.
- next: push branch, open PR, merge, verify production; confirm the SWW side ships in step so both sites gain the site-wide divider together.

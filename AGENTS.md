...
- Every closeout must state exactly one `JOURNAL` disposition: `REPO-LOG updated: <heading or path>`; `Covered by existing entry: <heading/link>`; or `Not needed: <one-sentence reason>`.

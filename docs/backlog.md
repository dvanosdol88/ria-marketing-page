...
- [ ] **Delete unused `src/components/CostAnalysisCalculator.original.tsx`.** Backup copy with zero imports (verified 2026-07-31). It still contains the pre-#183 button-link pattern (button-styled anchor without `no-underline`), so blindly restoring it would resurrect the invisible-button bug fixed in #183. Confirm it isn't wanted as reference, then delete. *(added 2026-07-31)*

## Security & Supply Chain

- [ ] **Two breaking-change audit paths deferred (`postcss` + `uuid`).** After the non-breaking `npm audit fix` on 2026-06-17 (cleared both High vulns, dropped count 28→10) and a second pass on 2026-07-17 (fixed critical `websocket-driver` CVEs GHSA-mp7j-qc5w-4988 + GHSA-xv26-6w52-cph6 by bumping 0.7.4→0.7.5 in lock file; now **0 critical, 10 moderate**), the remaining 10 are all moderate and both require a major/breaking bump: (1) **`postcss <8.5.10`** is bundled inside Next (`node_modules/next/node_modules/postcss`) — `npm audit fix --force` would downgrade to `next@9.3.3`; wait for an upstream Next patch instead. (2) **`uuid <11.1.1`** is transitive via `firebase-admin`→`@google-cloud/*`/`google-gax`/`gaxios`/`teeny-request`/`retry-request` — the only fix npm offers is `firebase-admin@14` (major). `firebase-admin` is load-bearing (quiz vote API → Firestore; see item below), so the v14 upgrade must be a deliberate, separately-tested change. *(added 2026-06-17, updated 2026-07-17)*
...
- [ ] **Tag internal/test traffic in PostHog / skip localhost.** The public RIA Builder visit/scan totals now omit `?selftest=1` browsers (cookie + localStorage; see protocol). Remaining: a PostHog-wide internal-traffic marker and skipping PostHog init on localhost. *(added 2026-08-03; aggregate exclusion 2026-09-23)*
...
- [x] **UTM-only cross-domain handoffs.** Campaign visitors retain the available standard UTM fields when they continue from youarepayingtoomuch.com to smarterwaywealth.com. The Advanced Calculator link renders without calculator state, and the click-time allowlist removes calculator inputs, `variant`, identity/session values, and unknown query keys before navigation. Implemented and locally browser-verified on `codex/yapt-cross-site-assumption-privacy-20260826`; production proof remains part of the protected release gate. *(added 2026-08-03; corrected 2026-08-26)*
- [x] **2b content pending David's sign-off (regulatory wording):** account-minimum FAQ answer and state-eligibility sentence. David approved the drafts verbatim 2026-08-03; shipped on THIS site (FAQ entries `account-minimum` + `out-of-state`, eligibility sentence in ComplianceFooter). *(added 2026-08-03, done 2026-08-03)*
- [ ] **Mirror the 2b answers + footer eligibility sentence to smarter-way-wealth — HELD for David's explicit go.** ComplianceFooter's mirror rule applies; the sister site's FAQ lacks both answers. Do not ship without his confirmation (2026-08-03 scope correction limited work to this site). *(added 2026-08-03)*
